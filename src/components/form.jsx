...
export function Form({ className, children, workspace }) {
  return (
      <article className={classNames('Form', className, { Form_workspace: workspace })}>
        {children}
      </article>
  )
}
...
export function FormTitle({ level = 2, className, children, ...props }) {
  const Title = `h${level}`
  return (
      <Title className={classNames('Form__Title', className)} {...props}>{children}</Title>
  )
}
...
export function Input({ className, inputRef, disabled, ...props }) {
  return (
      <input className={classNames('Form__Input', className, {
...
export function ErasabledInput({ onChange, ...props }) {
  return (
    <div className={'Form__ErasableInput'}>
      ...
      <EraseIcon className={'Form__Eraser'} onClick={() => onChange && onChange({ target: { value: '' }})}/>
    </div>
  )
}
...
export function ButtonsRow({ className, children, disabled, final, label, ...props }) {
  return (
      <div className={classNames(className, 'Form__ButtonsRow')}>
        {children}
      </div>
  )
}

export function Button({ className, disabled, cancel, center, label, btnRef, ...props }) {
  return (
      <button className={classNames('Form__Button Form__Button_single', className, {
        Form__Button_disabled: disabled,
        Form__Button_cancel: cancel,
        Form__Button_center: center,
      })} ref={btnRef} {...props}>{label}</button>
  )
}
...
export function FileChoiceRow({ label, onFileChoice, value, ...props }) {
  return (
      <>
        <span className={'Form__Label'}>{label}</span>
        <input className={'Form__Input Form__Input_readonly Form__FilePreview'} value={value} title={value} readOnly/>
        <button className={'Form__Button Form__Button_file'} onClick={onFileChoice}><FolderIcon className={'Form__Icon'}/></button>
      </>
  )
}
