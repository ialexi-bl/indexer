export default {
  ...
  /* Help */
  'Info': 'Informacje',
  'We tried to guess your language automatically. Change it, if it is wrong. You can do it later using the "View" menu.': 'Twój język został ustawiony automatycznie. Zmień go, jeśli to konieczne. Możesz to zrobić później w menu „Widok”.',
  'about_article': `
...
<1>Uwaga! Ten program został stworzony przez amatora, i w związku z tym może zawierać błędy.</1>
`.trim(),
  'help_article': `
<0>Pomoc</0>
<1>Przestrzeń robocza</1>
<2>Przestrzeń robocza składa się z trzech części: pola dla ogólnych danych, informacji o mężu i o żonie. Przy zapisywaniu danego rekordu pole roku nie ulegnie zmianie, natomiast numer dokumentu zostanie zwiększony o jeden. Korzystaj z klawiszy<1><0></0></1><2></2><3><0></0></3><4></4><5><0></0></5>, aby przejść do następnego, i<7><0></0></7><8></8><9><0></0></9><10></10><11><0></0></11> - aby wrócić do poprzedniego pola. Jeśli przemieścisz się do przodu z zaznaczonym polem „Dodaj”, rekord zostanie automatycznie dodany do dokumentu.<2>
...
<10>Jak zostało już wcześniej wspomniane, możesz używać <1><0></0></1><2></2><3><0></0></3><4></4><5><0></0></5> i <7><0></0></7><8></8><9><0></0></9><10></10><11><0></0></11> do nawigacji między polami odpowiednio wprzód i w tył.</10>
<11>Skorzystaj z <13><0></0></13>, aby oznaczyć osobę jako zmarłą tam, gdzie jest to możliwe.</11>
<12>Możesz wyczyścić pole, nad którym pracujesz z pomocą <1><0></0></1>.</12>
<13>Kliknij na nazwę pola lub użyj <1><0></0></1>, aby oznaczyć, że nie jesteś pewny co do zawartości pola.</13>
<14>Zapisz tabelę, używając <1><0></0></1>.</14>
`.trim(),
  ...
  'Contains headers (Don\'t change to auto detect)': '”Zawiera nagłówki (Nie wprowadzaj zmian, żeby określić automatycznie)”',
  /* Выбор, в каком порядке в файле расположены колонки */
  'Drag to specify, in which order the columns are arranged in your file': 'Przesuwaj, aby ustalić kolejność kolumn w pliku',
  ...
  'If you continue, all unsaved changes will be lost. Do you want to save them?': 'Przy kontynuacji wszystkie niezapisane dane zostaną utracone. Czy chcesz je zapisać?',
  ...
  'An error happened while creating a file': 'Przy próbie utworzenia pliku wystąpił błąd',
  
  'Drag to specify the order of the input fields.': 'Przesuwaj, aby ustalić kolejność pól do wprowadzania danych',
}
